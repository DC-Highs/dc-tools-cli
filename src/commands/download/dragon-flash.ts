...
export const dragonFlashCommand = new Command("dragon-flash")
    ...
    .action(async (argImageName, options) => {
        try {
            ...
            const outputFile = options.output || `${imageName}_phase${phase}_flash.swf`

            await ensureDirectory(outputFile)
            const downloader = dcAssets.dragons.animations.flash({
                imageName,
                phase,
                skin: options.skin,
            ...
            logSuccess(`Dragon Flash animation saved to ${outputFile}`)
        } catch (error) {
            logError("Failed to download dragon Flash animation", error)
        }
    })
